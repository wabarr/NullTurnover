...
## Bibi results chi square exclude zero bins

```{r}

bibiResults <- sapply(1:1000, FUN=function(x) detectPulsesChiSq(treeDepth = 2.5, criterion = 1.5, nTaxa=52, desiredBinNumber = 10, deathRate =  0.8533333, excludeZeroes = TRUE))

sum(bibiResults > 0) / length(bibiResults)
qplot(factor(bibiResults), geom="bar", position="stack") +  
  labs(x="number of pulses") + 
  annotate(x=2.03, y=600, geom="text", label=paste0(100*round(sum(bibiResults > 0) / length(bibiResults),2),"% of simulations \ndetect at least one pulse"), size=8) + 
  labs(title="Number of pulses X^2: African bovid parameters")

```

## Calculate average birth and death of Bibi data

```{r}
bibiIntervals <- binTimeData(bibi[,c("FAD", "LAD")], int.length = 0.25)
bibiRates <- perCapitaRates(bibiIntervals, plot=FALSE)
desiredIntervals <- bibiRates[,"int.start"] %in% seq(3.75, 1.5, by=-0.25)
bibiSubset <- bibiRates[desiredIntervals,]
bibiP <- mean(bibiSubset[,"pRate"])
bibiQ <- mean(bibiSubset[,"qRate"])
```

## Simulate a tree with Bapst package paleotree
```{r}
bibiFossilRecord <- simFossilRecord(p=bibiP/0.25, q=bibiQ/0.25, totalTime = 7, nTotalTaxa = c(50, 300))
simTree <- taxa2phylo(fossilRecord2fossilTaxa(bibiFossilRecord))
```